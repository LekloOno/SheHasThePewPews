...
public class PMA_DriftControl : PMA_SurfaceControl<DATA_DriftControl>
{
    ...
    {
        //cacheHeight = spaceState.GroundHeight;
        cacheVelocity = -rb.velocity.y;
        if(/*cacheHeight < data.MaxHeight && */!spaceState.IsGrounded && cacheVelocity > 0 && Time.time-inputHandler.LastBackward < data.BackBuffer)
        {
