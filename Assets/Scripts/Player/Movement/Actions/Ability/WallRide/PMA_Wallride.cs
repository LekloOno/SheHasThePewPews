...
public class PMA_Wallride : PMA_Ability<DATA_Wallride>
{
    ...
    {
        if(!spaceState.IsGrounded && (Physics.Raycast(rb.transform.position, _playerDir.right, out _wall, data.WallMaxDistance, _ground) || Physics.Raycast(rb.transform.position, -_playerDir.right, out _wall, data.WallMaxDistance, _ground)))
        {
    ...
    void OnWallRide(object sender, EventArgs e)
    {
        ...
        if(_rideTracker > 0 && !spaceState.IsGrounded && (rb.transform.position - wallClosestPoint).magnitude < data.WallMaxDistance)
        {
